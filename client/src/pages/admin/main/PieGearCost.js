...
export default function PieGearCost() {
  const {
    state: { gears },
  } = useValue();
  ...
  useEffect(() => {
    let free = 0,
      ...
      moreThan35 = 0;
    gears.forEach((gear) => {
      if (gear.price === 0) return free++;
      if (gear.price < 15) return lessThan15++;
      if (gear.price <= 35) return between15And35++;
      moreThan35++;
    });
    setCostGroups([
      { name: 'Free Rental', qty: free },
  ...
  return (
    ...
    >
      ...
      <Stack gap={2}>
        <Typography variant="h6">Gear Cost</Typography>
